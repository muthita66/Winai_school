...
export function ProfileFeature({ session }: ProfileFeatureProps) {
    ...
    const getAdvisorText = () => {
        ...
        if (advisorsList.length > 0) {
            return advisorsList.map((a: any) => `${a.teacher_code || ""} ${a.first_name || ""} ${a.last_name || ""}`.trim()).join(", ");
        }
        return "-";
    };
    ...
    if (isLoading) {
        return (
            <div className="space-y-6">
                <div className="bg-gradient-to-br from-indigo-800 to-purple-900 rounded-3xl p-8 shadow-lg">
    ...
    return (
        <div className="space-y-6">
            {/* Hero Section */}
            <section className="bg-gradient-to-br from-indigo-800 to-purple-900 rounded-3xl p-8 text-white shadow-lg relative overflow-hidden flex flex-col md:flex-row gap-8 items-center md:items-start justify-between">
                <div className="relative z-10 flex-1">
                    ...
                    <p className="text-indigo-200">
                        อัปเดตข้อมูลส่วนตัวให้เป็นปัจจุบันเพื่อสิทธิประโยชน์ของคุณ
                    </p>
                </div>

                <div className="relative z-10 flex gap-4">
                    <div className="bg-white/10 backdrop-blur-md rounded-2xl p-5 border border-white/20 min-w-[140px]">
                        <div className="text-indigo-200 text-sm mb-1">รหัสนักเรียน</div>
                        <div className="text-2xl font-bold">{profile.student_code || "-"}</div>
                    </div>
                    <div className="bg-white/10 backdrop-blur-md rounded-2xl p-5 border border-white/20 min-w-[140px]">
                ...
                <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none z-0">
                    <div className="absolute -top-24 -left-24 w-64 h-64 bg-purple-500 rounded-full blur-3xl opacity-30"></div>
                    <div className="absolute -bottom-24 -right-24 w-64 h-64 bg-indigo-500 rounded-full blur-3xl opacity-40"></div>
                </div>
            </section>
                ...
                {!isEditing ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="flex gap-4 p-4 rounded-xl border border-slate-100 bg-slate-50">
                            <div className="bg-white w-10 h-10 rounded-full flex items-center justify-center text-indigo-500 shadow-sm shrink-0">
                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                            </div>
                            <div>
                        ...
                        <div className="flex gap-4 p-4 rounded-xl border border-slate-100 bg-slate-50">
                            ...
                            <div>
                                ...
                                <div className="text-slate-800 font-semibold">{advisor}</div>
                            </div>
                        </div>
                    ...
                    <form onSubmit={handleSubmit} className="bg-slate-50 p-6 rounded-2xl border border-slate-200">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={handleInputChange}
                                    className="w-full border border-slate-300 rounded-lg px-4 py-2.5 outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                    required
                                />
                            </div>
                            <div>
                            ...
                            <div className="col-span-1 md:col-span-2">
                                ...
                                <textarea
                                    ...
                                    onChange={handleInputChange}
                                    rows={3}
                                    className="w-full border border-slate-300 rounded-lg px-4 py-2.5 outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 resize-none"
                                />
                            </div>
                        </div>
                        ...
                        <div className="flex justify-end gap-3 border-t border-slate-200 pt-6">
                            ...
                            <button
                                type="submit"
                                disabled={updateProfileMutation.isPending}
                                className="px-6 py-2.5 rounded-xl bg-indigo-600 text-white font-medium hover:bg-indigo-700 transition-colors shadow-sm flex items-center justify-center gap-2 min-w-[120px]"
                            >
